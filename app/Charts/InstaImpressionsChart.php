...
class InstaImpressionsChart extends BaseChart
{
    /**
    ...
    public function handler(Request $request): Chartisan
    {
        ...
        if($request->comparisonBegin){
            ...
            foreach($instasCompare as $value){
                ...
                array_push($compare, $value->impressions);
            }
        }

        return Chartisan::build()
            ->labels($dates)
            ->dataset('Nombre impressions '.$label, $impressions)
            ->dataset('Nombre impressions '.$labelCompare, $compare);
