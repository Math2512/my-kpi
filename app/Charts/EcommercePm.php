...
class EcommercePm extends BaseChart
{
    /**
    ...
    public function handler(Request $request): Chartisan
    {
        ...
        if($request->comparisonBegin){
            ...
            foreach($ecommerceCompare as $value){
                ...
                array_push($compare, $value->pm);
            }
        }
        
        return Chartisan::build()
            ->labels($dates)
            ->dataset('Panier Moyen '.$label, $pm)
            ->dataset('Panier Moyen '.$labelCompare, $compare);
    }
}